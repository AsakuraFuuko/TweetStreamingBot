...
class LoginWithTwitter {
    ...
    login(user_id, cb) {
        const oauth = {
            ...
            callback: this.callbackUrl + '?tg_user_id=' + user_id
        };

        // Get a "request token"
        request.post({url: TW_REQ_TOKEN_URL, oauth: oauth, proxy: this.proxy}, (err, _, body) => {
    ...
    callback(params, tokenSecret, cb) {
        ...
        request.post({url: TW_ACCESS_TOKEN_URL, oauth: oauth, proxy: this.proxy}, (err, _, body) => {
            if (err) return cb(err);
